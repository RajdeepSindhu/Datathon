document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadInitialData() {
    try {
      ...
      if (healthRes.ok) {
        ...
        if (statMandis) statMandis.textContent = health.total_mandis || "57";
        if (statCrops) statCrops.textContent = health.total_crops || "7";
      }
  ...
  async function submitQuery(queryText) {
    ...
    try {
      ...
      thinkingEl.remove();
      appendErrorMessage("Error connecting to Mandi server. Please ensure the backend is running at http://localhost:8000.");
      console.error(err);
    } finally {
      isSubmitting = false;
  ...
  function appendBotResponse(resp) {
    ...
    summarySection.className = "ai-summary-block";
    summarySection.innerHTML = `
      ...
      <div class="ai-summary-text">${escapeHtml(resp.summary || resp.answer || "No summary available.")}</div>
    `;
    botCard.appendChild(summarySection);

    // 2. Applied Filters Badges
    if (resp.filters_applied && Object.keys(resp.filters_applied).length > 0) {
    ...
    if (resp.chart) {
      ...
      chartWrap.className = resp.chart.type === "kpi" ? "kpi-container" : "chart-wrapper";
    ...
    if (resp.data && resp.data.length > 0 && (!resp.chart || resp.chart.type !== "table")) {
      ...
      let trs = resp.data.slice(0, 25).map(r => {
